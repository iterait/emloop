"""
Test module for train check hook (cxflow.hooks.train_check_hook).
"""
...
from cxflow.hooks.train_check_hook import TrainCheckHook
...
class TrainCheckHookTest(CXTestCase):
    ...
    def test_stream_raise(self):
        ...
        hook = TrainCheckHook(_VAR, _MIN_ACCURACY, _MAX_EPOCH, "unknown")
    ...
    def test_variable_raise(self):
        ...
        hook = TrainCheckHook("not_present", _MIN_ACCURACY, _MAX_EPOCH)
        with self.assertRaises(KeyError):
            hook.after_epoch(_CURRENT_EPOCH, self._epoch_data)

    def test_not_scalar_raise(self):
        """Test raising error, when variable is not scalar."""
        hook = TrainCheckHook(_VAR, _MIN_ACCURACY, _MAX_EPOCH)
    ...
    def test_training_terminated(self):
        ...
        """
        hook = TrainCheckHook(_VAR, _MIN_ACCURACY, _MAX_EPOCH, "train")
    ...
    def test_epoch_raise(self):
        ...
        """
        hook = TrainCheckHook(_VAR, _MIN_ACCURACY, _MAX_EPOCH, "test")
        with self.assertRaises(ValueError):
