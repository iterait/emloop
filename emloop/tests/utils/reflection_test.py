"""
Test module for reflection utils (emloop.utils.reflection).
"""
import sys
import os
import pytest

from emloop.utils.reflection import create_object, find_class_module, get_class_module, parse_fully_qualified_name
...
class ImportedClass:  # pylint: disable=missing-docstring
    # this class is imported in emloop.tests.utils.dummy_module
    pass
...
def test_create_object():
    ...
    module_name = 'emloop.tests.utils.reflection_test'
...
INVALID_PARAMETERS = [
    ({}, TypeError),
    ({'module_name': 'emloop.tests.utils.reflection_test'}, TypeError),
    ({'class_name': 'SimpleClass'}, TypeError),  # insufficient arguments
    ({'module_name': 123, 'class_name': 'SimpleClass'}, AssertionError),
    ({'module_name': 'emloop.tests.utils.reflection_test', 'class_name': 123}, AssertionError),  # wrong argument types
    ({'module_name': 'emloop.tests.utils.reflection_test_xxx', 'class_name': 'SimpleClass'}, ImportError),
    ({'module_name': 'emloop.tests.utils.reflection_test', 'class_name': 'WrongName'},
...
FIND_MODULES = [(['SimpleClass'], ['emloop.tests.utils.reflection_test']),  # correct setup
                (['DoesNotExists'], []),  # non-existent class
                (['DuplicateClass'],
                 ['emloop.tests.utils.reflection_test', 'emloop.tests.utils.dummy_module'])]  # multiple matches
...
def test_find_class_module(params, expected_modules):
    ...
    matched_modules, erroneous_modules = find_class_module('emloop.tests.utils', *params)
...
def test_get_class_module():
    ...
    module = get_class_module('emloop.hooks', 'LogProfile')
    expected_module = 'emloop.hooks.log_profile'
    assert module == expected_module

    # test if None is returned when the class is not found
    module2 = get_class_module('emloop.hooks', 'IDoNotExist')
    expected_module2 = None
    assert module2 == expected_module2

    # test if exception is raised when multiple modules are matched
    with pytest.raises(ValueError):
        get_class_module('emloop.tests.utils', 'DuplicateClass')
    ...
    module3 = get_class_module('emloop.tests.utils', 'ImportedClass')
    possible_submodules = {'emloop.tests.utils.reflection_test', 'emloop.tests.utils.dummy_module'}
