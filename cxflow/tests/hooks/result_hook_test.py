"""
Test module for result hook (cxflow.hooks.result_hook).
"""
import logging
...
class ResultHookTest(TestCase):
    ...
    def test_result_with_train(self):
        ...
        hook.after_batch('test', {'a': np.array([12, 13, 14]), 'b': np.array([120, 130, 140]),
                                  'c': np.array([0, 9, 1])})
        hook.after_batch('test', {'a': np.array([15, 16, 17]), 'b': np.array([150, 160, 170]),
                                  'c': np.array([2, 3, 1])})
