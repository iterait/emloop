...
class DatasetLoaderTest(TestCase):
    ...
    def test_verify_dataset(self):
        ...
        # NoTest
        try:
            DatasetLoader(config, '').load_dataset()
        except:
            self.fail()

        config['dataset']['dataset_class'] = 'SampleDatasetNoTrain'
        self.assertRaises(ValueError, DatasetLoader(config, '').load_dataset)

        config['dataset']['dataset_class'] = 'SampleDatasetNoValid'
        self.assertRaises(ValueError, DatasetLoader(config, '').load_dataset)
        ...
        config['dataset']['dataset_class'] = 'SampleDatasetNoTest'
        self.assertRaises(ValueError, DatasetLoader(config, '').load_dataset)
        ...
        # NoTest
        try:
            config['dataset']['dataset_class'] = 'SampleDatasetTest'
            DatasetLoader(config, '').load_dataset()
        except:
            self.fail()
